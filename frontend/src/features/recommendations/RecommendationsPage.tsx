      ...
      <div className="space-y-8 pb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-[28px] font-bold text-[#3A2E27]">Live Opportunity Scout</h1>
            </div>
            <p className="text-[15px] text-[#5C5C52] max-w-2xl">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 px-4 py-2 rounded-full font-bold text-sm transition-colors border border-emerald-500/20 disabled:opacity-50"
          >
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-[13px] font-bold transition-all whitespace-nowrap ${
                  activeFilter === type 
                    ...
                    : 'bg-transparent border border-[#333333] text-[#5C5C52] hover:text-[#3A2E27] hover:border-[#666666]'
                }`}
              >
                {type}
              </button>
          ...
          <div className="relative w-full md:w-80 flex-shrink-0">
            ...
            <input 
              ...
              className="w-full bg-white border-[1.5px] border-[#3A2E27] text-[#3A2E27] placeholder:text-[#5C5C52] rounded-full pl-10 pr-4 py-2 focus:outline-none focus:border-[#1D9E75]/50 transition-colors text-sm"
            />
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="text-[#5C5C52] text-sm">Loading your opportunities...</p>
          </div>
        ) : opportunities.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 border border-dashed border-[#333333] rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.06)] bg-[#0a0a0a]">
            <Compass className="w-12 h-12 text-[#666666] opacity-50 mb-4" />
            <h3 className="text-[#3A2E27] font-bold mb-2">No Live Opportunities</h3>
            <p className="text-[#5C5C52] text-sm mb-6 text-center max-w-md">Your agent hasn't found any active opportunities yet. Trigger a live search to scan the internet.</p>
            <button 
              onClick={handleRefresh}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            {filteredData.map((item) => (
              <div 
                key={item.id}
                className="bg-white border-[1.5px] border-[#3A2E27] rounded-[16px] shadow-[0_2px_8px_rgba(0,0,0,0.06)] hover:-translate-y-[3px] hover:shadow-[0_6px_16px_rgba(0,0,0,0.08)] transition-all duration-200 overflow-hidden animate-fade-in-up transition-all flex flex-col group"
              >
                {/* Header Row */}
                <div className="p-5 pb-4 border-b-[1.5px] border-[#3A2E27] bg-white">
                  <div className="flex justify-between items-start gap-4 mb-3">
                    <h3 className="text-[17px] font-bold text-[#3A2E27] line-clamp-2 leading-tight">
                      {item.title}
                    </h3>
                    <div className="flex flex-col items-end shrink-0 gap-1">
                       <span className="bg-emerald-500/10 text-emerald-500 text-[10px] font-bold px-2 py-0.5 rounded-[6px] border border-emerald-500/20">
                         {Math.round(item.confidence_score * 100)}% Match
                       </span>
                       {item.priority_score > 0.8 && (
                         <span className="bg-orange-500/10 text-orange-500 text-[10px] font-bold px-2 py-0.5 rounded-[6px] border border-orange-500/20">
                           High Priority
                         </span>
                       )}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-[12px] font-medium">
                    <span className="text-[#3A2E27] bg-black/5 border border-[#3A2E27]/20 px-2 py-1 rounded-[6px]">{item.category}</span>
                    <span className="flex items-center gap-1 text-[#5C5C52]">
                ...
                <div className="p-5 flex-1 flex flex-col bg-white">
                  ...
                  <p className="text-[14px] text-[#5C5C52] mb-5 line-clamp-3 leading-relaxed">
                    {item.description}
                  </p>
                  
                  {/* AI Explanation Box */}
                  <div className="mt-auto mb-5 bg-white rounded-[12px] p-4 border-[1.5px] border-[#3A2E27]">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                      <span className="text-[11px] font-bold text-emerald-500 tracking-wider uppercase">AI Analysis</span>
                    </div>
                    <p className="text-[13px] text-[#3A2E27] mb-2 leading-relaxed">
                      "{item.ai_explanation}"
                    </p>
                    <div className="flex items-start gap-2 text-[12px] text-[#5C5C52] pt-2 border-t border-[#3A2E27]/20 mt-2">
                      <span className="font-bold text-[#3A2E27]">Impact:</span>
                  ...
                  <div className="flex justify-between items-center pt-2">
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className="p-2.5 rounded-[10px] text-[#5C5C52] hover:text-[#3A2E27] hover:bg-black/5 transition-colors group/btn relative"
                      >
                        <X className="w-4 h-4" />
                      ...
                      <button 
                        ...
                        className="p-2.5 rounded-[10px] text-[#5C5C52] hover:text-rose-500 hover:bg-rose-50 transition-colors group/btn relative"
                      >
                    ...
                    <a 
                      ...
                      className="flex items-center gap-2 bg-[#1FA35A] text-white hover:bg-[#198549] px-5 py-2.5 rounded-full text-[13px] font-bold transition-colors"
                    >
                      View Opportunity
