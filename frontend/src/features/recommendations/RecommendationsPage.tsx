    ...
    <DashboardLayout>
      <div className="space-y-10">
        ...
        <div>
          ...
          <p className="text-[15px] text-[#9CA3AF]">Everything DASKALOS has curated for you, media matched to where you are right now.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
                  activeFilter === type 
                    ...
                    : 'bg-transparent border border-[#1F2937] text-[#9CA3AF] hover:text-white hover:border-white/20'
                }`}
              >
                {type}
              </button>
          ...
          <div className="relative w-full md:w-64 flex-shrink-0">
            ...
            <input 
              ...
              className="w-full bg-[#0B0F1A] border border-[#1F2937] text-white placeholder-[#9CA3AF] rounded-lg pl-10 pr-12 py-[8px] focus:outline-none focus:border-[#6366F1] focus:ring-1 focus:ring-[#6366F1] transition-colors text-sm"
            />
        ...
        {isEmpty ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[#9CA3AF] text-sm">Your recommendations will appear here as DASKALOS gets to know you.</p>
          </div>
        ) : (
          /* Grid */
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredData.map((item, idx) => (
              <div 
                key={idx}
                className="bg-[#131826] border border-[#1F2937] rounded-xl overflow-hidden hover:-translate-y-[2px] hover:border-[#6366F1]/50 transition-all duration-150 ease-out group flex flex-col"
              >
                {/* Thumbnail Block */}
                <div className="h-[140px] w-full bg-[#1F2937]/30 flex items-center justify-center group-hover:bg-[#6366F1]/10 transition-colors">
                  <span className="opacity-20">
                     {/* Abstract icon based on type could go here, or just stay as a solid block */}
                  </span>
                </div>
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <h3 className="text-[16px] font-medium text-white mb-1 line-clamp-2">{item.title}</h3>
                  <p className="text-[13px] text-[#9CA3AF] mb-4">{item.source}</p>
                  
                  {/* Reasoning */}
                  <div className="mt-auto mb-4 border-l-2 border-[#1F2937] pl-3 py-0.5">
                    <p className="text-[13px] text-[#9CA3AF] italic line-clamp-2 leading-relaxed">
                      "{item.reasoning}"
                    </p>
                  </div>
