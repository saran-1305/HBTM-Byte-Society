...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white border border-[#333333] p-3 rounded-full shadow-xl">
        <p className="text-[#3A2E27] font-bold mb-1">{label}</p>
        <p className="text-[#5C5C52] text-sm">Stage: <span className="text-[#3A2E27] capitalize">{STAGE_LABELS[data.stageName]}</span></p>
        <p className="text-[#5C5C52] text-xs mt-1 italic max-w-[220px]">"{data.reason}"</p>
      </div>
    );
  }
  return null;
  ...
  return (
    <div className="animate-fade-in">
      <div className="space-y-8">
        {/* Header */}
        <div>
          <h1 className="text-[28px] font-bold text-[#3A2E27] mb-2">Analytics</h1>
          <p className="text-[15px] text-[#5C5C52]">How your journey has actually moved, not how much time you've spent.</p>
        </div>

        {/* Top Stat Row */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-6 rounded-[16px] flex flex-col justify-between min-h-[140px]">
            <div>
              <p className="text-sm font-bold text-[#5C5C52] mb-1">Current stage</p>
              <h2 className="text-[24px] font-bold text-[#3A2E27] leading-tight capitalize">{currentStage || 'Unknown'}</h2>
              <p className="text-[12px] text-[#5C5C52] mt-1">{ordinal(currentStageIndex + 1)} of {STAGE_NAMES.length} stages</p>
            </div>
          ...
          <div className="bg-white p-6 rounded-[16px] flex flex-col justify-between min-h-[140px]">
            <div>
              <p className="text-sm font-bold text-[#5C5C52] mb-1">Stage transitions</p>
              <h2 className="text-[24px] font-bold text-[#3A2E27] leading-tight">{history.length}</h2>
            </div>
            <p className="text-[12px] text-[#5C5C52] mt-4 leading-relaxed">
          ...
          <div className="bg-white p-6 rounded-[16px] flex flex-col justify-between min-h-[140px]">
            <div>
              <p className="text-sm font-bold text-[#5C5C52] mb-1">Latest evaluation confidence</p>
              <h2 className="text-[24px] font-bold text-[#3A2E27] leading-tight capitalize">{confidence || 'N/A'}</h2>
            </div>
            <p className="text-[12px] text-[#5C5C52] mt-4">
        ...
        <div className="bg-white p-6 rounded-[16px]">
          <h2 className="text-[16px] font-bold text-[#3A2E27] mb-6">Journey over time</h2>
          {journeyData.length === 0 ? (
            <div className="h-[200px] flex items-center justify-center">
              <p className="text-[#5C5C52] text-sm">No stage transitions yet — this fills in as ARC moves you through your journey.</p>
            </div>
          ) : (
          ...
          <div className="flex items-center gap-6 mt-6 pt-4 border-t border-[#333333]">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[12px] text-[#5C5C52]">Stage transition (real, AI-evaluated)</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-[16px]">
          <h2 className="text-[15px] font-bold text-[#3A2E27] mb-6">Evidence breakdown</h2>
          {totalObs === 0 ? (
            <p className="text-[#5C5C52] text-sm">No observations recorded yet.</p>
          ) : (
                ...
                {Object.entries(obsBySource).map(([source, count], idx) => (
                  <div key={source} className="flex justify-between items-center text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[#5C5C52] capitalize">{source}</span>
                    </div>
                    <span className="text-[#3A2E27] font-bold">{Math.round((count / totalObs) * 100)}%</span>
                  </div>
                ))}
              </div>
              <p className="text-[11px] text-[#3A2E27]/30 pt-2">Based on your {totalObs} most recent recorded observations.</p>
            </div>
          )}
        </div>
